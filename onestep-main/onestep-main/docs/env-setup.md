...
- For the current production database, both values should point to Neon Postgres.
...
| `DATABASE_URL` | No | Neon -> Project -> Dashboard -> Connection Details -> Prisma or Postgres URI. | Use a Neon Postgres URI in the form `postgresql://[NEON_USER]:[NEON_PASSWORD]@[NEON_HOST]/[NEON_DATABASE]?sslmode=require&channel_binding=require`. |
| `DIRECT_URL` | No | Neon -> Project -> Dashboard -> Connection Details -> Direct/Prisma URI. | Required by Prisma for migrations and direct database access. Use the same Neon URI format. If NanoCorp only exposes one Neon URI in the shell, set `DIRECT_URL` to the same value as `DATABASE_URL`. |
...
Set these exact placeholder formats in Vercel, replacing the bracketed values with the real Neon credentials:

```bash
DATABASE_URL=postgresql://[NEON_USER]:[NEON_PASSWORD]@[NEON_HOST]/[NEON_DATABASE]?sslmode=require&channel_binding=require
DIRECT_URL=postgresql://[NEON_USER]:[NEON_PASSWORD]@[NEON_HOST]/[NEON_DATABASE]?sslmode=require&channel_binding=require
```

Notes:

- Keep the full query string suffix so SSL and channel binding stay enabled for Neon.
- Do not wrap the values in extra quotes when pasting into the Vercel dashboard.
- In this NanoCorp environment, the available live database URL already resolves to a Neon host, so the checked-in app should treat Neon as the source of truth for Prisma.
...
- The Neon database setup for this task also requires `DIRECT_URL` to be present anywhere Prisma migrations or seeding will run.
- Remaining variables in this guide still need real values from Stripe, Supabase, Neon, and Resend before they can be configured.
